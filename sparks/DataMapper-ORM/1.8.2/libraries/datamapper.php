...
class DataMapper implements IteratorAggregate {
	...
	public function __get($name)
	{
		...
		if($name == 'form_validation')
		{
			...
			{
				//XXX GAB autoload form_validation fix
	...
	public function validate($object = '', $related_field = '')
	{
		...
		foreach ($this->validation as $field => $validation)
		{
			...
			if ($related || $this->_force_validation || ! isset($this->stored->{$field}) || $this->{$field} !== $this->stored->{$field} || array_key_exists('conditional_required', $rules))
			{
				...
				foreach ($rules as $rule => $param)
				{
					...
					if (is_string($line) || $result === FALSE)
					{
						...
						if (is_array($param))
						{
							...
							//XXX GAB
							if ($rule === 'conditional_required'){
								$params = array();
								foreach($param as $p){
									$params[] = $this->validation[$p]['label'];
								}
								$param = $params;
							}
							...
							{
								//XXX GAB
								//$param = substr_replace($param, ' | ', $pos, 2);
								$param = substr_replace($param, ' | ', $pos, 2);
							}
						}
	 ...
	 * @ignore
	 */
	protected function _related_required($object, $model, $id)
	{
