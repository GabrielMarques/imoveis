...
class Carabiner {
	...
	public function __construct(){
		...
		if (!file_exists($this->cache_path)){
			show_error('Carabiner: cache path does not exist. Please set the cache path in config/carabiner.php.');
		}
		if (!is_writable($this->cache_path))
		{
			show_error('Carabiner: cache path is not writable. Please update the permissions of ' . $this->cache_dir .'.');
		}
	...
	private function _less($input_file, $output_file = ''){
		...
		//XXX GAB Edit
		if (array_key_exists($input_file, $this->bootstrap_files)){
			$check_files = $this->bootstrap_files[$input_file];
			foreach($check_files as $check_file){
				if (file_exists($this->cache_path . $output_file) === false || filemtime($this->style_path . $check_file) > filemtime($this->cache_path . $output_file)){
					$less = new lessc($this->style_path . $input_file);
					file_put_contents($this->cache_path . $output_file, $less->parse());
					break;
				}
			}
		}else{
			lessc::ccompile($this->style_path . $input_file, $this->cache_path . $output_file);
		}
	...
	private function _display_js($group = 'main'){
		...
		} else if($this->combine && $this->minify_js) {
	...
	private function _display_css($group = 'main'){
		...
		} else if($this->combine && $this->minify_css) { // we're combining and minifying
		...
		} else if($this->combine && !$this->minify_css) { // we're combining bot not minifying
		...
		} else if(!$this->combine && $this->minify_css) { // we want to minify, but not combine
