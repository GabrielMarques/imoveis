...
abstract class OAuth_Provider {
	...
	public function __construct(array $options = NULL)
	{
		...
		if ( ! $this->name)
		{
			...
			$this->name = ucfirst(substr(get_class($this), strlen('Provider_')));
		}
	}
	...
	public function signed_request($url, OAuth_Consumer $consumer, OAuth_Token $token, array $params = NULL)
	{
		$access = array('oauth_consumer_key' => $consumer->key, 'oauth_token' => $token->access_token);
		$params = $params === null ? $access : array_merge($params, $access);

		// Create a new GET request with the required parameters
		$request = OAuth_Request::forge('resource', 'GET', $url, $params);
