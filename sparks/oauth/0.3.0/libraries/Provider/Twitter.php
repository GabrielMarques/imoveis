...
class OAuth_Provider_Twitter extends OAuth_Provider {
	...
	public function get_user_info(OAuth_Consumer $consumer, OAuth_Token $token)
	{
		...
		if (isset($user->screen_name) === false){
			throw new Exception('Provider user info error');
		}
		...
		return array(
			...
			'fullname' => $user->name ? $user->name : $user->screen_name,
			//'location' => $user->location,
			'image' => 'https://api.twitter.com/1/users/profile_image?screen_name=' . $user->screen_name . '&size=bigger',
