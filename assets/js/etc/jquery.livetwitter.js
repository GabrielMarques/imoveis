      ...
      // ..if not, let's initialize.
        ...
        this.twitter = {
          ...
          renderTweet: function (tweet) {
            var html = '<tr class="tweet tweet-' + tweet.id + '">';
            
            if (this.settings.showAuthor) {
              html += '<td><img width="' + this.settings.imageSize + '" height="' + this.settings.imageSize + '" src="' + tweet.profile_image_url + '" /></td>';
              html += '<td><h5><a class="username" href="' + tweet.profile_url + '" rel="external">' + tweet.screen_name + '</a></h5>';
            } else {
              html += '<td>';
            }
            html += '<p class="text">' + this.parseText(tweet.text) + '</p>';
            html += '</td>';

            if (this.settings.timeLinks) {
              html += '<td><span class="time pull-right">';
              html += '<a href="' + tweet.url + '" rel="external">';
              html += this.relativeTime(tweet.created_at);
              html += '</a></span></td>';
            } else {
              html += '<span class="time">' + this.relativeTime(tweet.created_at) + '</span></td>';
            }
            ...
            if (twitter.settings.refresh || initialize) {
              ...
              $.getJSON(twitter.apiURL(), function (json) {
                ...
                if (newTweets > 0) {
                  ...
                  $(twitter.container).find('tr.tweet:gt(' + (twitter.settings.limit - 1) + ')').remove();
